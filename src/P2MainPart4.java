...
public class P2MainPart4 {
    ...
    */
    ...
    private static Dataset removeFeature(Dataset ds, int index) {
        double x[][] = ds.getX();
        double newx[][] = new double[ds.getSize()][ds.getInputDims() - 1];
        for(int i = 0; i < ds.getSize(); i++) {
            for(int j = 0; j < ds.getInputDims(); j++) {
                if(j < index) {
                    newx[i][j] = x[i][j];
        ...
        return new Dataset(newx, ds.getY());
    }

    private static void run(JSONObject settings, boolean preprocess, Random rnd, int i) {
        if(i != -1)
            trainset = removeFeature(trainset, i);
        ...
        try{
            ...
            if (preprocess)
                preprocess_testset();
            if(i != -1)
                testset = removeFeature(testset, i);
    ...
    public static void main(String[] args){
        ...
        Dataset trainset_temp = trainset;
        Dataset testset_temp = testset;
        run(settings, preprocess, rnd, -1);
        int indices = trainset.getInputDims();
        for(int i = 0; i < indices; i++) {
            System.out.println("------ running " + i + " ------");
            trainset = trainset_temp;
            testset = testset_temp;
            run(settings, preprocess, rnd, i);
        }
	}
}
