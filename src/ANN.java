...
public class ANN {
    ...
    */
	public Layer build(int input_dims, int output_dims, int n_hidden_layers, int n_nodes_per_hidden_layer, String activation_function){
        ...
        Layer[] layers = new Layer[n_hidden_layers * 2 + 2];
        layers[0] = new Linear(input_dims, n_nodes_per_hidden_layer, new Linear.WeightInitXavier());
        if(n_hidden_layers == 0) layers[0] = new Linear(input_dims, output_dims, new Linear.WeightInitXavier());

        for(int i = 1; i < n_hidden_layers * 2; i+=2) {
            layers[i] = act;
            if(i + 1 == n_hidden_layers * 2) {
                layers[i + 1] = new Linear(n_nodes_per_hidden_layer, output_dims, new Linear.WeightInitXavier());
            } else {
                layers[i + 1] = new Linear(n_nodes_per_hidden_layer, n_nodes_per_hidden_layer, new Linear.WeightInitXavier());
            }
        }

        layers[n_hidden_layers * 2 + 1] = new Softmax();
